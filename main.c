...
int main(int argc, char *argv[]){
  ...
  if (strcmp(argv[1],"-t") == 0){                                                                                                     //mode test: si le deuxi̬me argument dans la ligne de commande est -t
    char* value = argv[2];                                                                                                            //r̩cup̬re dans un char* value le deuxi̬me argument
    int k = atoi(value);                                                                                                              //la fonction atoi() permet de convertir le char* value en entier dans k
    char* tab_files[6]= {"alea4-6.txt", "alea4-12.txt","alea10-40.txt",
             "alea50-1000.txt","alea100-5000.txt","alea1000-10000.txt"};                                                              //tableau contenant l'ensemble des fichiers test
    for (int i = 0; i<=5; i++){
  ...
  else if (strcmp(argv[1],"-n") == 0){                                                                                                //mode normal: si le deuxi̬me argument dans la ligne de commande est -n
    ...
    algorithm_wiki(file,x);                                                                                                          //applique l'algorithme de pagerank au fichier 
  }
  
  
  else {
    printf("Veuillez consulter la notice READ ME.\n");                                                                                 //si les arguments ne sp̩cifie pas le mode affichage d'une consigne
  }
  
 
  time_t end = time(NULL);                                                                                                              //fin du chronom̬tre
