...
void graph_init_mat(FILE *f, List Adj[],List Adj_pred[], int n){                                         //Remplit le graphe de Adj, sans la page du SuperNoeud, et le graphe de Adj_pred
    ...
    for (int i=1; i<=n; i++){                                                                            //Parcourt les lignes
     ...
     for(int j=0; j<n; j++){                                                                              //Parcourt sur les colonnes
      ...
      Adj[i].cpt_sort = cpt;                                                                   
      add_list(&Adj[i],value);                                                                              //S'ajoute lui même
...
Page* recup_vec(FILE* f, int n){                                                                            //On se trouve à la ligne avant les Vecteurs Résultat
  Page* Vec_Res = malloc(sizeof(Page)*(n+1));                                            
  char line[256];
  fgets(line, sizeof(line), f);                                                                             //Permet d'être à la ligne des vecteurs résultats                                                                                   

  for(int i = 0; i<n+1; i++){
    if (i == 0) Vec_Res[i].name = "SuperNode";                                                              //Initialise le nom de Vec_Res[0] à SuperNodde
    else{
      char* value = malloc(SIZE_BUFFER_name);                                             
      sprintf(value,"%d",i);
      Vec_Res[i].name = value;                                                                              //Initialise les noms des Vec_Res de 1 à n+1
    }
    fscanf(f,"%Lf, ", &Vec_Res[i].pageRank);                                                                //Permet de récuperer chaque nombre sous la forme d'un long double et le met dans le pageRank du Vec_Res 
  }
  return Vec_Res;
}


void moy_max(Page Vec_Ex[], Page Vec_Res[], int n){                                                         //Calcul la moyenne et le maximum des écarts entre les Vecteurs Resultats et les Vecteurs Experimentaux
    float ecart_max[n+1];                                                                                   //Créer tableau des écarts maximum
    float moy=0;  
    ecart_max[0]=(fabs((Vec_Ex[0].pageRank - Vec_Res[0].pageRank)/Vec_Res[0].pageRank))*100;                //fabs() : valeur absolue pour les double
  ...
  moy=moy/(n+1);                                                                                            //divise la somme des ecart_max par le nombre de valeurs sommé, c'est à dire n+1
...
void experience(char* filename, int k){
  ...
  int n = size_matrix(f);
  List Adj[n+1];
  rewind(f);                                                                                                 //Recommence à lire le fichier du départ
  List Adj_pred[n+1];
